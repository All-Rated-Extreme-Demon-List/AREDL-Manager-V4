import { guildId, staffGuildId, maxPointsOnShiftMiss, enableSeparateStaffServer, missedShiftsID, maxPoints, defaultPoints } from '@/../config.json';
import { api } from '@/api';
...
export const handle = async (client: Client<true>, data: ShiftMissedData) => {
	Logger.log('Received shift missed notification:');
	Logger.log(data);
	...
	for (const shift of data.aredl) {
		...
		if (reviewer.discord_id) {
			...
			if (recordsDone >= (2/3)) {
				newPoints = Math.max(points.points - (maxPointsOnShiftMiss * 1/3), 0);
			} else if (recordsDone >= (1/3)) {
				newPoints = Math.max(points.points - (maxPointsOnShiftMiss * 2/3), 0);
			} else {
			...
			Logger.warn(`Shift Missed - Reviewer ${shift.user_id} has no Discord ID, skipping points decrement.`);
		}
		// unix epochs
		let startDate = Math.floor(new Date(shift.start_at).getTime() / 1000);
