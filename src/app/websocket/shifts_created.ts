import { guildId, staffGuildId, maxPointsOnShiftMiss, enableSeparateStaffServer, shiftsStartedID } from '@/../config.json';
import { api } from '@/api';
...
export const handle = async (client: Client<true>, data: WebsocketShift[]) => {
	Logger.log('Received shifts created notification:');
	Logger.log(data);
	...
	for (const shift of data) {
		try {
			...
			const dbShift = await db.insert(shiftNotificationsTable).values({
			...
			setTimeout(async () => {
				await sendShiftNotif(channel as TextChannel, dbShift).catch((err: any) => {
