import { guildId, staffGuildId, completedShiftsID, enableSeparateStaffServer, pointsOnShiftComplete, maxPoints, defaultPoints } from '@/../config.json';
import { api } from '@/api';
...
export const notification_type = "SHIFT_COMPLETED";

export const handle = async (client: Client<true>, data: WebsocketShift) => {
	Logger.log('Received shift completed notification:');
	Logger.log(data);
	...
	// unix epochs
	let startDate = Math.floor(new Date(data.start_at).getTime() / 1000);
	let endDate = Math.floor(new Date(data.end_at).getTime() / 1000);
