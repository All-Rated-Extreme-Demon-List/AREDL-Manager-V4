...
export default task({
	...
	async execute({ client }) {
        ...
        const classicSubmissionsRes = await api.send<PaginatedResponse<Submission>>(
            '/aredl/submissions',
            'GET',
        ...
        const platSubmissionsRes = await api.send<PaginatedResponse<Submission>>(
            '/arepl/submissions',
            'GET',
            { per_page: 999, status_filter: 'UnderConsideration' },
            undefined,
        ...
        await submissions.sort((a, b) => b.updated_at.getTime() - a.updated_at.getTime());
        ...
        alreadyReminded = alreadyReminded.filter((reminded) => {
            ...
            if (!submissionExists) {
                db.delete(sentUcRemindersTable).where(eq(sentUcRemindersTable.id, reminded.id))
                return false;
            }
            return true;
        });
        ...
        for (const submission of submissions) {
            ...
            const now = new Date();
            const diff = Math.round((now.getTime() - updatedAt.getTime()) / 1000 / 3600 / 24);
