...
export default task({
	...
	async execute({ client }) {
		...
		let allShiftsRes = await getShifts(oneWeekAgo);
		if (!allShiftsRes) {
			Logger.error("Error getting shifts");
			return;
		}
		const allShifts = allShiftsRes.reduce((obj, shift) => {
		...
		const isOddWeek = (await db.select({ count: count() }).from(weeklyMissedShiftsTable))[0]?.count === 0;
		...
		for (const [staffId, shifts] of Object.entries(allShifts)) {
			...
			if (allCompleted) {
				...
				if (!isOddWeek) {
					const missedLastShift = await db.select().from(weeklyMissedShiftsTable).where(eq(weeklyMissedShiftsTable.user, staffId)).limit(1).get()
		...
		if (sendWeeklyUpdates) {
			const channel = staffGuild.channels.cache.get(weeklyUpdatesChannelId);
