...
export const autocomplete: AutocompleteCommand = async ({ interaction }) => {
    ...
    if (focused.name === 'level') {
        let res = await api.send<Level[]>('/aredl/levels', 'GET', {
...
export const chatInput: ChatInputCommand = async ({ interaction }) => {
    ...
    if (subcommand === 'level') {
        ...
        let levelRes = await api.send<ExtendedLevel>(`/aredl/levels/${levelID}`, 'GET');
        if (levelRes.error) {
            Logger.error(
        ...
        let levelCreatorsRes = await api.send<BaseUser[]>(
        ...
        let row = new ActionRowBuilder().setComponents(go).toJSON();
        ...
        let lbRes = await api.send<LeaderboardEntry>('/aredl/leaderboard', 'GET', query);
        if (lbRes.error) {
            Logger.log(
        ...
        const container = (
            <Container accentColor={0xff6f00}>
                ...
                {leaderboard.map((entry) => {
                    ...
                    return (
                        <Section>
                            ...
                            <Button style={ButtonStyle.Link} url={`https://aredl.net/profiles/${entry.user.id}`}>
                                Go
                            </Button>
                        </Section>
                    )
                ...
                <ActionRow>
                    <Button style={ButtonStyle.Link} url={`https://aredl.net/leaderboard/`}>
                        Open In AREDL
                    </Button>
                </ActionRow>
            </Container>
